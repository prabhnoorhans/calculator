...
var sign; 
var valueLength;
var fontSize = window.getComputedStyle(result, null).getPropertyValue("font-size");
var fontSizeN = parseFloat(fontSize);
...
function input(n) {
    dLength = valueS.length;
    if ((Number.isInteger(n) == true || n == '.') && dLength < 16) {
        ...
        if (valueS.includes('e+') == true) {
            var x = dLength - valueS.indexOf("e+");
            console.log(x);
            console.log("(e+): true");
            dLength -= x;
            console.log(dLength);
        ...
        if (dLength > 10) {
            ...
            var twoDiff = diff * 2;
            console.log(twoDiff);
            result.style.fontSize = (fontSizeN - twoDiff) + 'px';
        } else if (dLength <= 10) {
